 ...
 * @param {String} options.businessId
 * @param {String} options.transactionId
 * @param {Boolean} options.noRemove
 * @param {Object} patch
 * @return {Transaction} transaction
 */
module.exports.updateTransaction = function updateTransaction(options, patch, next) {
  var hasErr = helpers.hasKeys(options, ['businessId', 'transactionId']);
  if (hasErr) {
    return next(hasErr);
  }
  ...
  }, function (oldTransaction) {
    if (!oldTransaction) {
      return next(helpers.error('NotFoundError', 'Old transaction not found', 404));
    }

    patch = helpers.patch(oldTransaction, patch, {
      noRemove: typeof options.noRemove !== 'undefined' ? options.noRemove : true
    });

  }, function () {
    self.request({
      url: '/businesses/' + encodeURIComponent(options.businessId) + '/transactions/' + encodeURIComponent(
...
/**
 * Refunds a transaction on a business.
 * @param {String} options.businessId
 * @param {String} options.transactionId
 * @return {Transaction} transaction
 */
module.exports.refundTransaction = function createProduct(options, next) {
  var hasErr = helpers.hasKeys(options, ['businessId', 'transactionId']);
  if (hasErr) {
    return next(hasErr);
  }
  ...
  var f = ff(function () {
    ...
    if (!transaction) {
      return next(helpers.error('NotFoundError', 'Transaction not found', 404));
    }
    ...
    self.request({
      ...
      body: {
        ...
        context: {
          businessId: transaction.context.businessId,
          storeId: transaction.context.storeId,
          storeDeviceId: transaction.context.storeDeviceId
        },
        fundingSource: {
          type: transaction.fundingSource.type
        },
        // amount: "1.00"
      }
    }, f.slot());
...
/**
 * Captures a transaction on a business.
 * @param {String} options.businessId
 * @param {String} options.transactionId
 * @return {Transaction} transaction
 */
module.exports.captureTransaction = function createProduct(options, next) {
  var hasErr = helpers.hasKeys(options, ['businessId', 'transactionId']);
  if (hasErr) {
    return next(hasErr);
  }
  ...
  var f = ff(function () {
    ...
    self.request({
      url: '/businesses/' + encodeURIComponent(options.businessId) + '/transactions/' +
        encodeURIComponent(options.transactionId) + '/capture',
      method: 'POST',
      body: {
